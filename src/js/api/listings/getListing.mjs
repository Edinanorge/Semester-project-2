...
export async function getEndingSoonListings() {
  try {
    const response = await fetch(
      `${BASE_URL}/listings?_seller=true&_bids=true&_active=true&sort=endsAt&sortOrder=asc&limit=8`,
      {
        method: "GET",
        headers: headers(),
      }
...
export async function getListing(id) {
  try {
    ...
    const response = await fetch(`${BASE_URL}/listings/${id}?_seller=true&_bids=true`, {
      method: "GET",
      headers: headers(),
    });
